...
module.exports = (client) => {
    if (lavalinkConfig.enabled) {
        ...
        client.on('voiceStateUpdate', async (oldState, newState) => {
            try {
                // 24/7 mode: if bot was kicked/disconnected, rejoin instead of cleanup
                if (oldState.member.id === client.user.id && oldState.channelId && !newState.channelId) {
                    let is247 = false;
                    try {
                        const { sessions247 } = require('../commands/lavalink/247');
                        const session = sessions247.get(oldState.guild.id);
                        if (session) {
                            is247 = true;
                            setTimeout(async () => {
                                try {
                                    const vcChannel = client.channels.cache.get(session.voiceChannelId);
                                    if (!vcChannel) return;
                                    let player = client.riffy.players.get(oldState.guild.id);
                                    if (player) {
                                        // reconnect existing player
                                        player.voiceChannel = session.voiceChannelId;
                                        player.connect();
                                    } else {
                                    ...
                                    console.log(`[247] Rejoined VC in guild ${oldState.guild.id}`);
                                } catch (e) {
                                    console.error('[247] Rejoin failed:', e.message);
                                }
                            }, 2000);
                        }
                    } catch (e) {}
